...
public class OrderLiniaRutaPNavison {
	...
	public Calendar m_DataPrevistaInterior_o, m_DataPrevistaCobertes_o;
	public Calendar m_DataJobTicketFet_o;
	...
	public OrderLiniaRutaPNavison() {
		m_semaphoreMutex = new Semaphor(1);
		this.setDefault();
	}
	...
	public String getNLogs() {
		...
		builder.append("</b>");
		builder.append(" | Dta últim canvi: ");
		builder.append("<b> ");
		builder.append(getDataUltimCanvi());
		builder.append("</b>");
		int n = 3;
	...
	public void setDefault() {
		...
		m_modfLinia = new Vector<LogsModificacions>();
		m_format = new SimpleDateFormat("EEE,d MMM yyyy HH:mm");
	...
	public String getPaper() {
		...
		if (this.matcher("KOMORI?", this.m_nI_Nom_Maquina_l) || this.matcher("KBA?", this.m_nI_Nom_Maquina_l)) {
			...
			String str_aux = bobina;
			if (!l_paper.isEmpty()) {
				str_aux = bobina.substring(bobina.indexOf(' ') != -1 ? bobina.indexOf(' ') : 0, bobina.length());
			}
			l_builder.append(str_aux);
		} else if (this.matcher("HEIDELBERG?", this.m_nI_Nom_Maquina_l)) {
			l_builder.append(this.m_qualitatPaperMaquinaFulla_l).append("_").append(this.m_GramatgePaperMaquinaFulla_l);
		} else {
	...
	public String getData() {
		String l_dataFormated = " ";
		if (m_DataPrevistaCobertes_o != null) {
			l_dataFormated = m_format.format(m_DataPrevistaCobertes_o.getTime());
		} else {
	...
	public String getDataInteriors() {
		String l_dataFormated = " ";
		if (m_DataPrevistaInterior_o != null) {
			l_dataFormated = m_format.format(m_DataPrevistaInterior_o.getTime());
		}
		return l_dataFormated;
	}

	public String getDataJobTicket() {
		String l_dataFormated = " ";
		if (m_DataJobTicketFet_o != null) {
			l_dataFormated = m_format.format(m_DataJobTicketFet_o.getTime());
		}
		return l_dataFormated;
	}

	public String getDataUltimCanvi() {
		Calendar l_data = getDataUltimCanviCalendar();
		String l_strData = "";
		if (l_data != null)
			l_strData = m_format.format(l_data.getTime());
		return l_strData;
	}

	public Calendar getDataUltimCanviCalendar() {
		Calendar l_data = m_modfLinia.size() > 0 ? m_modfLinia.get(0).getDataCalendar() : null;
		return l_data;
	}
	...
	public String getDataHora() {
		return new StringBuilder().append(this.getData()).append(" ").append("").toString();
	...
	public String getM_uI_Estado_r() {
		return m_uI_Estado_r.length() > 4 ? m_uI_Estado_r.substring(0, 4) + "." : m_uI_Estado_r;
	}
	...
	public String formaHTML(String content) {
		StringBuilder builder = new StringBuilder();
		String l_maquina = "";
		l_maquina += this.m_tipusProducteMaquinaFulla_l.trim().equals("") ? ""
				: " - " + this.m_tipusProducteMaquinaFulla_l;
	...
	public String getTirada() {
		...
		return String.valueOf(decimalFormat.format((int) str.doubleValue()));
	}
	...
	public String getM_nI_No_Planes_Totals_Tiratge_l() {
		return this.m_nI_No_Planes_Totals_Tiratge_l;
	}
	...
	public String getM_nI_Plegat_Tiratge_l() {
		return this.m_nI_Plegat_Tiratge_l;
	}

	public String getFormatPlegat() {
		StringBuilder l_builder = new StringBuilder();
		l_builder.append(m_nI_Plegat_Tiratge_l).append(m_nI_Format_Tiratge_l.trim() == "" ? ""
				: m_nI_Plegat_Tiratge_l.trim() != m_nI_Format_Tiratge_l.trim() ? "" : "/" + m_nI_Format_Tiratge_l);
		return l_builder.toString();
	}
	...
	public String getToolTipDataHora() {
		StringBuilder l_builder = new StringBuilder();
		l_builder.append("<html>");
		l_builder.append("<b>").append("Data Covertes: ").append("</b>").append(getDataCovertes());
		l_builder.append("<br>");
		l_builder.append("<b>").append("Data Interiors: ").append("</b>").append(getDataInteriors());
		l_builder.append("</html>");
		return l_builder.toString();
	}

	public boolean esMenorDtaJobTicket() {
		Calendar l_data = getDataUltimCanviCalendar();
		if (l_data != null) {
			return m_DataJobTicketFet_o.before(getDataUltimCanviCalendar());
		}
		return false;
	}
