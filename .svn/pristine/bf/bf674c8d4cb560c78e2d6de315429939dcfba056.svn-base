...
public class LogsModificacionsDAO {
	...
	public Vector<LogsModificacions> Get(int p_lan) {
		LogsModificacions l_finalResult = null;

		Vector<LogsModificacions> l_res = new Vector<>();

		if (m_bdd.executaSQLSELECT(
				"SELECT " + m_sentencia + " FROM logs_modificacions WHERE id = " + p_lan + " ORDER BY id ASC ") > 0) {
